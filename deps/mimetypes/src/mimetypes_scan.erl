...
-module(mimetypes_scan).

-export([string/1,string/2,token/2,token/3,tokens/2,tokens/3]).
-export([format_error/1]).

%% User code. This is placed here to allow extra attributes.
-file("src/mimetypes_scan.xrl", 18).
...
-file("src/mimetypes_scan.erl", 277).
yystate() -> 3.
...
yystate(3, [9|Ics], Line, Tlen, Action, Alen) ->
    yystate(5, Ics, Line, Tlen+1, Action, Alen);
yystate(3, [C|Ics], Line, Tlen, Action, Alen) when C >= 45, C =< 57 ->
    yystate(0, Ics, Line, Tlen+1, Action, Alen);
...
-file("src/mimetypes_scan.xrl", 9).
yyaction_0(TokenChars, TokenLine) ->
     { token, { string, TokenLine, TokenChars } } .

-compile({inline,yyaction_1/1}).
-file("src/mimetypes_scan.xrl", 10).
yyaction_1(TokenLine) ->
     { token, { newline, TokenLine } } .

-compile({inline,yyaction_2/0}).
-file("src/mimetypes_scan.xrl", 12).
yyaction_2() ->
     skip_token .

-compile({inline,yyaction_3/0}).
-file("src/mimetypes_scan.xrl", 13).
yyaction_3() ->
     skip_token .
